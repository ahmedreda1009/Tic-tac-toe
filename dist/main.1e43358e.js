...
var movesDivs = document.querySelectorAll('.moves .moves-block button');
var winnerPlyer = document.querySelector('.info .winner span');
var nextPlayerField = document.querySelector('.info .next-player span');
var moves = document.querySelector('.moves .moves-block');

function isPlayerWon(arr) {
  var winArra = [];
  var result = winCases.some(function (element) {
    var res = element.every(function (ele) {
...
function CheckWinner(clickedSquare) {
  ...
  if (currentPlayer === 'X') {
    playerXMoves.push(+clickedSquare.dataset.id); // currentPlayer = 'O';
    ...
    playerOMoves.push(+clickedSquare.dataset.id); // currentPlayer = 'X';
  ...
  var hisObj = {
    x: _toConsumableArray(playerXMoves),
    o: _toConsumableArray(playerOMoves),
    next: nextPlayerField.textContent,
    winnerP: winnerPlyer.textContent
  };
  history.push(hisObj);
  var moveBlock = document.createElement('button');
  var moveText = document.createTextNode("Go to move #".concat(movesCounter + 1));
  moveBlock.classList = 'btn btn-primary d-block mb-2';
  moveBlock.dataset.id = movesCounter + 1;
  moveBlock.addEventListener('click', function (e) {
